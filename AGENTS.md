...
- **조달청 표준교재가 공식 자료이며 모든 학습자료의 기준이다.** 표준교재와 현행 법령·고시의 값이 다르면 **표준교재 값을 본문에 두고**, 현행값은 `현행(시행일)`으로 부기한다. 개정된 현행값을 삭제하지 않는다.
- 표기는 `표준교재 기준: …` / `현행(시행일): …` / `표준교재 외 보충`으로 통일한다.
- **표준교재 안에서 서술이 엇갈리는 항목**은 어느 쪽도 기준값이 될 수 없으므로 예외적으로 현행 규정 원문을 따르고, 그 사유와 교재의 두 서술을 함께 적는다.
