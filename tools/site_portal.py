...
def render_portal(rounds: list[int], lectures: list[LectureLink] | None = None) -> str:
    ...
    wrong_cbt_section = (
        ...
        '<div><h2>누적 오답 CBT</h2><p>틀린 문제만 모아 다시 풀고, 정답을 맞히면 자동 제거</p></div></div>'
...
@media(min-width:1024px){{body{{font-size:18px}}.hero{{padding:5rem 3rem 4.2rem}}.hero-inner,.page{{max-width:1520px}}h1{{font-size:3.7rem}}.hero p{{max-width:900px;font-size:1.3rem}}.quick-nav{{margin-top:2rem;gap:.8rem}}.quick-nav a{{padding:.75rem 1.2rem;font-size:1rem}}.page{{padding:2rem 2.5rem 6rem}}.section{{padding:3rem 0}}.section-head{{margin-bottom:1.6rem}}.section h2{{font-size:2rem}}.section-head p{{font-size:1.08rem}}.choice-grid{{grid-template-columns:repeat(2,minmax(0,1fr));gap:1.25rem}}.choice-card{{min-height:190px;padding:1.5rem;border-radius:16px}}.card-kicker{{font-size:.95rem}}.choice-card strong{{font-size:1.75rem;margin:.45rem 0 .25rem}}.choice-card>span:last-of-type{{font-size:1.08rem}}.subject-group{{margin:1.15rem 0;border-radius:15px}}.subject-group summary{{padding:1.45rem 1.6rem;font-size:1.2rem}}.subject-group summary small{{font-size:1rem}}.lecture-body{{padding:0 1.6rem 1.5rem}}.lecture-part{{padding:1.2rem 0}}.lecture-part h3{{font-size:1.18rem}}.chapter-grid{{grid-template-columns:repeat(3,minmax(0,1fr));gap:.8rem}}.chapter-grid a{{padding:1rem 1.05rem}}.chapter-grid strong{{font-size:1.05rem}}.review-link{{padding:1.15rem 1.4rem}}.footer{{font-size:.95rem}}}}
</style>
</head>
<body>
