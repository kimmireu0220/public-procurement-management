...
def _study_groups() -> str:
    ...
    for subject, parts in STUDY_PARTS.items():
        ...
        groups.append(
            f'<details class="subject-group"{" open" if subject == 1 else ""}>'
...
def _lecture_groups(links: list[LectureLink]) -> str:
    ...
    for subject, items in sorted(by_subject.items()):
        ...
        subject_groups.append(
            f'<details class="subject-group lecture-group" open><summary>'
...
*{{box-sizing:border-box}}html{{scroll-behavior:smooth}}body{{margin:0;background:var(--bg);color:var(--text);font-family:"Apple SD Gothic Neo","Malgun Gothic",sans-serif;line-height:1.55}}
a{{color:inherit}}.hero{{background:linear-gradient(135deg,#102f50,#1763a6);color:#fff;padding:3.4rem 1.25rem 2.8rem}}.hero-inner,.page{{max-width:1040px;margin:0 auto}}
