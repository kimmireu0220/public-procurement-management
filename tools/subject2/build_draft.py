"""stable_id 목록으로 2과목 _draft/2과목_선별.md 생성."""
...
from subject2.bank import fetch_question
from subject2.selection import SELECTIONS
...
def build_draft(round_no: int, stable_ids: list[str] | None = None) -> Path:
    ...
    if not ids:
        raise SystemExit(f"no stable_id selection for 2과목 round {round_no}")

    out = SUBJECT2.round_dir(round_no) / "_draft"
    out.mkdir(parents=True, exist_ok=True)
    path = out / DRAFT_NAME

    lines = [
        f"# 2과목 전용 모의 {round_no}회차 — 선별 ({len(ids)}문항)\n",
