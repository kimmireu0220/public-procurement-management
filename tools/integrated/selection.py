"""통합 필기 모의 회차별 수동 선별 stable_id 목록."""

from __future__ import annotations

# 4회차 — 통합 1~3회차 + 과목 단독 manifest의 stable_id 미사용(3과목 Part2/3 exam 제외)
# 오답노트 약점: 1(전자조달·낙성·부정당) · 2(RFQ/RFP·사전규격·협상순서)
#                 3(보증금·MAS·물가·하도급)
# 3과목 Part2/3 exam 풀 고갈 → §A-0b 2차 lap: 3:2:*·3:3:* exam 15건 재출제 (LAP2_REUSE_3)

# Part 분포 5/4/5/4/5/4/3 · check 3건(≤6)
SUBJECT1_ROUND4: list[str] = [
    # Part1 (5)
    "1:1:1:exam:11",
    "1:1:1:exam:17",
    "1:1:2:exam:8",
    "1:1:3:exam:14",
    "1:1:4:exam:5",
    # Part2 (4) — 전자조달·ERA
    "1:2:1:exam:16",
    "1:2:2:exam:9",
    "1:2:2:exam:23",
    "1:2:1:exam:10",
    # Part3 (5) — 전자조달·나라장터·전자입찰/계약
    "1:3:1:exam:3",
    "1:3:1:exam:21",
    "1:3:2:exam:21",
    "1:3:2:exam:22",
    "1:3:2:exam:3",
    # Part4 (4)
    "1:4:1:exam:1",
    "1:4:1:exam:10",
    "1:4:2:exam:10",
    "1:4:3:exam:2",
    # Part5 (5)
    "1:5:1:exam:11",
    "1:5:1:exam:13",
    "1:5:2:exam:2",
    "1:5:3:exam:1",
    "1:5:4:exam:1",
    # Part6 (4) — check 3: 계약유형·전자입찰취소·수의계약 (낙성/요물 exam 풀 소진)
    "1:6:2:check:5",
    "1:6:4:check:2",
    "1:6:5:check:1",
    "1:6:6:exam:13",
    # Part7 (3) — 부정당업자
    "1:7:1:exam:36",
    "1:7:2:exam:23",
    "1:7:1:exam:35",
]

# Part 분포 5/5/5/5 · check 2건(≤4)
SUBJECT2_ROUND4: list[str] = [
    # Part1 (5) — check 1
    "2:1:1:exam:10",
    "2:1:2:exam:26",
    "2:1:2:exam:11",
    "2:1:1:exam:11",
    "2:1:2:check:2",
    # Part2 (5)
    "2:2:1:exam:7",
    "2:2:2:exam:11",
    "2:2:3:exam:10",
    "2:2:4:exam:9",
    "2:2:4:exam:10",
    # Part3 (5) — RFQ·RFP·사전규격
    "2:3:1:exam:29",
    "2:3:2:exam:7",
    "2:3:2:exam:8",
    "2:3:2:exam:12",
    "2:3:3:exam:10",
    # Part4 (5) — 협상·절차순서 · check 1 (RFQ/RFP 대응)
    "2:4:1:exam:9",
    "2:4:2:exam:14",
    "2:4:3:exam:5",
    "2:4:3:exam:8",
    "2:4:1:check:1",
]

# Part 분포 8/7/8/7 · cqa 0건(≤6) · 빈출 cluster 각 2건
SUBJECT3_ROUND4: list[str] = [
    # Part1 (8) — 보증금 cluster 2건
    "3:1:4:exam:22",
    "3:1:4:exam:24",
    "3:1:4:exam:3",
    "3:1:4:exam:7",
    "3:1:4:exam:31",
    "3:1:4:exam:34",
    "3:1:4:exam:35",
    "3:1:4:exam:36",
    # Part2 (7) — 2nd lap exam · 물가 cluster 2건
    "3:2:2:exam:11",
    "3:2:2:exam:13",
    "3:2:2:exam:14",
    "3:2:1:exam:4",
    "3:2:1:exam:10",
    "3:2:2:exam:30",
    "3:2:2:exam:8",
    # Part3 (8) — 2nd lap exam · MAS cluster 2건
    "3:3:2:exam:17",
    "3:3:2:exam:18",
    "3:3:2:exam:5",
    "3:3:2:exam:6",
    "3:3:2:exam:7",
    "3:3:1:exam:1",
    "3:3:1:exam:2",
    "3:3:2:exam:3",
    # Part4 (7) — 하도급 cluster 2건
    "3:4:3:exam:34",
    "3:4:3:exam:41",
    "3:4:1:exam:28",
    "3:4:1:exam:33",
    "3:4:3:exam:13",
    "3:4:2:exam:30",
    "3:4:1:exam:37",
]

# 3과목 Part2/3 exam 2차 lap 재출제 (15건) — 출제_방향.md에 기록
LAP2_REUSE_3: list[str] = [
    "3:2:1:exam:4",
    "3:2:1:exam:10",
    "3:2:2:exam:8",
    "3:2:2:exam:11",
    "3:2:2:exam:13",
    "3:2:2:exam:14",
    "3:2:2:exam:30",
    "3:3:1:exam:1",
    "3:3:1:exam:2",
    "3:3:2:exam:3",
    "3:3:2:exam:5",
    "3:3:2:exam:6",
    "3:3:2:exam:7",
    "3:3:2:exam:17",
    "3:3:2:exam:18",
]

# Part 분포 5/4/5/4/5/4/3 · check 5건(≤6)
SUBJECT1_ROUND5: list[str] = [
    # Part1 (5) — 지방계약법 check 1
    "1:1:1:exam:12",
    "1:1:2:exam:5",
    "1:1:3:exam:5",
    "1:1:4:check:2",
    "1:1:4:exam:8",
    # Part2 (4) — 경쟁적 대화
    "1:2:2:exam:11",
    "1:2:2:exam:7",
    "1:2:2:exam:15",
    "1:2:1:exam:11",
    # Part3 (5)
    "1:3:1:exam:13",
    "1:3:2:exam:5",
    "1:3:2:exam:10",
    "1:3:3:exam:8",
    "1:3:3:exam:12",
    # Part4 (4)
    "1:4:3:exam:3",
    "1:4:1:exam:12",
    "1:4:2:exam:5",
    "1:4:5:exam:10",
    # Part5 (5)
    "1:5:5:exam:16",
    "1:5:1:exam:14",
    "1:5:2:exam:8",
    "1:5:3:exam:2",
    "1:5:4:exam:2",
    # Part6 (4) — check only (exam 풀 소진)
    "1:6:6:check:1",
    "1:6:1:check:4",
    "1:6:3:check:1",
    "1:6:4:check:3",
    # Part7 (3) — 무효·부정당
    "1:7:1:exam:37",
    "1:7:1:exam:40",
    "1:7:2:exam:25",
]

# Part 분포 5/5/5/5 · check 2건(≤4)
SUBJECT2_ROUND5: list[str] = [
    # Part1 (5)
    "2:1:1:exam:12",
    "2:1:2:exam:14",
    "2:1:2:exam:20",
    "2:1:2:exam:29",
    "2:1:2:exam:33",
    # Part2 (5) — 일반관리비
    "2:2:3:exam:9",
    "2:2:3:exam:19",
    "2:2:2:exam:9",
    "2:2:3:exam:12",
    "2:2:3:exam:2",
    # Part3 (5) — 사전규격
    "2:3:2:exam:11",
    "2:3:2:exam:2",
    "2:3:1:exam:15",
    "2:3:1:exam:20",
    "2:3:4:exam:4",
    # Part4 (5) — 지방 협상·평가·제안 후 협상
    "2:4:1:exam:8",
    "2:4:1:check:3",
    "2:4:2:check:2",
    "2:4:3:exam:9",
    "2:4:2:exam:9",
]

# Part 분포 8/4/2/16 · cqa 6건(≤6) — Part2/3 exam 고갈
SUBJECT3_ROUND5: list[str] = [
    # Part1 (8) — 이행보증·손해보험 잔여 exam 전량
    "3:1:4:exam:25",
    "3:1:4:exam:26",
    "3:1:4:exam:37",
    "3:1:4:exam:38",
    "3:1:4:exam:39",
    "3:1:4:exam:40",
    "3:1:4:exam:41",
    "3:1:4:exam:42",
    # Part2 (4) — cqa: 입찰공고·물가
    "3:2:1:cqa:3",
    "3:2:1:cqa:17",
    "3:2:2:cqa:10",
    "3:2:2:cqa:16",
    # Part3 (2) — cqa: PQ·MAS
    "3:3:1:cqa:22",
    "3:3:2:cqa:19",
    # Part4 (16) — 하도급·공사관리
    "3:4:1:exam:2",
    "3:4:1:exam:36",
    "3:4:1:exam:23",
    "3:4:1:exam:38",
    "3:4:1:exam:40",
    "3:4:1:exam:41",
    "3:4:3:exam:27",
    "3:4:3:exam:17",
    "3:4:3:exam:19",
    "3:4:3:exam:18",
    "3:4:3:exam:26",
    "3:4:3:exam:29",
    "3:4:3:exam:35",
    "3:4:3:exam:42",
    "3:4:3:exam:44",
    "3:4:2:exam:22",
]

# Part 분포 5/4/5/4/5/4/3 · check 6건(≤6)
SUBJECT1_ROUND6: list[str] = [
    # Part1 (5)
    "1:1:1:exam:26",
    "1:1:2:exam:11",
    "1:1:3:exam:9",
    "1:1:4:exam:6",
    "1:1:4:exam:20",
    # Part2 (4) — 경쟁적 대화
    "1:2:2:exam:20",
    "1:2:2:exam:12",
    "1:2:2:exam:17",
    "1:2:3:exam:14",
    # Part3 (5)
    "1:3:1:exam:5",
    "1:3:1:exam:16",
    "1:3:3:exam:4",
    "1:3:4:exam:6",
    "1:3:4:exam:11",
    # Part4 (4)
    "1:4:3:check:3",
    "1:4:5:exam:3",
    "1:4:5:exam:11",
    "1:4:1:exam:4",
    # Part5 (5)
    "1:5:4:exam:12",
    "1:5:2:exam:26",
    "1:5:2:exam:22",
    "1:5:1:exam:17",
    "1:5:5:exam:12",
    # Part6 (4) — check only (exam 풀 소진)
    "1:6:1:check:3",
    "1:6:5:check:3",
    "1:6:4:check:1",
    "1:6:2:check:2",
    # Part7 (3)
    "1:7:2:exam:8",
    "1:7:1:check:7",
    "1:7:2:exam:13",
]

# Part 분포 5/5/5/5 · check 2건(≤4)
SUBJECT2_ROUND6: list[str] = [
    # Part1 (5)
    "2:1:2:exam:13",
    "2:1:2:exam:15",
    "2:1:2:exam:22",
    "2:1:1:exam:6",
    "2:1:2:exam:16",
    # Part2 (5)
    "2:2:1:exam:1",
    "2:2:1:exam:3",
    "2:2:3:exam:5",
    "2:2:3:exam:14",
    "2:2:4:check:6",
    # Part3 (5)
    "2:3:1:exam:16",
    "2:3:1:exam:21",
    "2:3:3:exam:7",
    "2:3:4:exam:6",
    "2:3:1:exam:13",
    # Part4 (5)
    "2:4:1:check:2",
    "2:4:3:exam:19",
    "2:4:4:exam:1",
    "2:4:6:exam:17",
    "2:4:3:exam:4",
]

# Part 분포 5/4/5/16 · cqa 6건(≤6) — 2차 lap: 재사용 exam 9건 포함
SUBJECT3_ROUND6: list[str] = [
    # 잔여 미사용 exam 15
    "3:4:1:exam:3",
    "3:4:1:exam:4",
    "3:4:1:exam:25",
    "3:4:1:exam:32",
    "3:4:1:exam:39",
    "3:4:3:exam:22",
    "3:4:3:exam:23",
    "3:4:3:exam:31",
    "3:4:3:exam:33",
    "3:4:3:exam:36",
    "3:4:3:exam:37",
    "3:4:3:exam:38",
    "3:4:3:exam:39",
    "3:4:3:exam:40",
    "3:4:3:exam:43",
    # 2차 lap 재사용 exam 9
    "3:2:1:exam:4",
    "3:4:1:exam:37",
    "3:3:2:exam:26",
    "3:3:2:exam:1",
    "3:3:2:exam:12",
    "3:3:1:exam:35",
    "3:1:4:exam:17",
    "3:4:1:exam:22",
    "3:3:1:exam:5",
    # cqa 6 (상한)
    "3:1:4:cqa:12",
    "3:1:4:cqa:13",
    "3:2:1:cqa:6",
    "3:2:2:cqa:14",
    "3:2:2:cqa:6",
    "3:3:1:cqa:1",
]

# Part 분포 5/4/5/4/5/4/3 · check 4건(≤6)
SUBJECT1_ROUND7: list[str] = [
    # Part1 (5)
    "1:1:1:exam:13",
    "1:1:2:exam:12",
    "1:1:3:exam:10",
    "1:1:4:exam:10",
    "1:1:2:exam:13",
    # Part2 (4)
    "1:2:1:exam:12",
    "1:2:2:exam:1",
    "1:2:3:exam:1",
    "1:2:1:exam:13",
    # Part3 (5)
    "1:3:1:exam:10",
    "1:3:2:exam:1",
    "1:3:3:exam:10",
    "1:3:4:exam:1",
    "1:3:2:exam:11",
    # Part4 (4)
    "1:4:1:exam:13",
    "1:4:2:exam:11",
    "1:4:3:exam:4",
    "1:4:5:exam:1",
    # Part5 (5)
    "1:5:1:exam:15",
    "1:5:2:exam:10",
    "1:5:3:exam:10",
    "1:5:4:exam:10",
    "1:5:5:exam:1",
    # Part6 (4) — exam 풀 소진으로 check
    "1:6:1:check:1",
    "1:6:1:check:6",
    "1:6:1:check:8",
    "1:6:2:check:3",
    # Part7 (3)
    "1:7:1:exam:10",
    "1:7:2:exam:1",
    "1:7:1:exam:11",
]

# Part 분포 5/5/5/5 · check 0건
SUBJECT2_ROUND7: list[str] = [
    # Part1 (5)
    "2:1:1:exam:9",
    "2:1:2:exam:12",
    "2:1:2:exam:17",
    "2:1:2:exam:18",
    "2:1:2:exam:19",
    # Part2 (5)
    "2:2:3:exam:13",
    "2:2:3:exam:15",
    "2:2:3:exam:16",
    "2:2:4:exam:1",
    "2:2:4:exam:2",
    # Part3 (5)
    "2:3:1:exam:10",
    "2:3:1:exam:11",
    "2:3:3:exam:1",
    "2:3:3:exam:11",
    "2:3:4:exam:2",
    # Part4 (5)
    "2:4:1:exam:10",
    "2:4:2:exam:1",
    "2:4:3:exam:1",
    "2:4:4:exam:14",
    "2:4:6:exam:1",
]

# Part 분포 8/7/8/7 · cqa 6건(≤6)
# 3과목은 1차 lap 미사용 풀이 cqa만 남아 2차 lap 재출제(exam) 포함.
SUBJECT3_ROUND7: list[str] = [
    # Part1 (8)
    "3:1:3:exam:16",
    "3:1:4:exam:1",
    "3:1:4:exam:2",
    "3:1:4:exam:6",
    "3:1:4:exam:23",
    "3:1:4:exam:28",
    "3:1:4:exam:30",
    "3:1:4:exam:32",
    # Part2 (7)
    "3:2:1:exam:44",
    "3:2:1:exam:45",
    "3:2:1:exam:46",
    "3:2:1:exam:49",
    "3:2:1:exam:50",
    "3:2:2:exam:41",
    "3:2:2:exam:42",
    # Part3 (8)
    "3:3:1:cqa:14",
    "3:3:1:cqa:15",
    "3:3:2:cqa:1",
    "3:3:2:cqa:12",
    "3:3:2:cqa:16",
    "3:3:2:cqa:17",
    "3:3:2:exam:21",
    "3:3:2:exam:20",
    # Part4 (7)
    "3:4:1:exam:31",
    "3:4:1:exam:16",
    "3:4:1:exam:27",
    "3:4:2:exam:11",
    "3:4:2:exam:20",
    "3:4:3:exam:8",
    "3:4:3:exam:25",
]

# Part 분포 5/4/5/4/5/4/3 · check 0건
SUBJECT1_ROUND8: list[str] = [
    # Part1 (5)
    "1:1:1:exam:14",
    "1:1:2:exam:1",
    "1:1:3:exam:11",
    "1:1:4:exam:11",
    "1:1:1:exam:18",
    # Part2 (4)
    "1:2:1:exam:14",
    "1:2:2:exam:13",
    "1:2:3:exam:10",
    "1:2:1:exam:15",
    # Part3 (5)
    "1:3:1:exam:11",
    "1:3:2:exam:12",
    "1:3:3:exam:11",
    "1:3:4:exam:10",
    "1:3:1:exam:12",
    # Part4 (4)
    "1:4:1:exam:14",
    "1:4:2:exam:12",
    "1:4:3:exam:8",
    "1:4:4:exam:10",
    # Part5 (5)
    "1:5:1:exam:16",
    "1:5:2:exam:11",
    "1:5:3:exam:11",
    "1:5:4:exam:11",
    "1:5:5:exam:10",
    # Part6 (4) — exam 잔여 활용
    "1:6:1:exam:1",
    "1:6:2:exam:10",
    "1:6:3:exam:1",
    "1:6:4:exam:1",
    # Part7 (3)
    "1:7:1:exam:13",
    "1:7:2:exam:11",
    "1:7:1:exam:14",
]

# Part 분포 5/5/5/5 · check 1건(≤4)
SUBJECT2_ROUND8: list[str] = [
    # Part1 (5)
    "2:1:2:exam:21",
    "2:1:2:exam:23",
    "2:1:2:exam:24",
    "2:1:2:exam:25",
    "2:1:2:exam:30",
    # Part2 (5) — check 1
    "2:2:1:check:2",
    "2:2:3:exam:17",
    "2:2:4:exam:4",
    "2:2:3:exam:18",
    "2:2:4:exam:6",
    # Part3 (5)
    "2:3:1:exam:12",
    "2:3:3:exam:12",
    "2:3:4:exam:5",
    "2:3:1:exam:18",
    "2:3:3:exam:14",
    # Part4 (5)
    "2:4:1:exam:11",
    "2:4:2:exam:3",
    "2:4:3:exam:10",
    "2:4:4:exam:15",
    "2:4:5:exam:1",
]

# Part 분포 8/7/8/7 · cqa 6건(≤6) · **2차 lap** exam 24건 재출제
SUBJECT3_ROUND8: list[str] = [
    # Part1 (8) — 6 exam + 2 cqa
    "3:1:1:exam:1",
    "3:1:1:exam:10",
    "3:1:1:exam:11",
    "3:1:1:exam:12",
    "3:1:1:exam:13",
    "3:1:1:exam:14",
    "3:1:3:cqa:3",
    "3:1:3:cqa:4",
    # Part2 (7) — 6 exam + 1 cqa
    "3:2:1:exam:1",
    "3:2:1:exam:10",
    "3:2:1:exam:11",
    "3:2:1:exam:12",
    "3:2:1:exam:13",
    "3:2:1:exam:14",
    "3:2:1:cqa:1",
    # Part3 (8) — 6 exam + 2 cqa
    "3:3:1:exam:1",
    "3:3:1:exam:10",
    "3:3:1:exam:11",
    "3:3:1:exam:12",
    "3:3:1:exam:13",
    "3:3:1:exam:14",
    "3:3:1:cqa:12",
    "3:3:1:cqa:13",
    # Part4 (7) — 6 exam + 1 cqa
    "3:4:1:exam:1",
    "3:4:1:exam:10",
    "3:4:1:exam:11",
    "3:4:1:exam:12",
    "3:4:1:exam:13",
    "3:4:1:exam:14",
    "3:4:1:cqa:1",
]

# Part 분포 5/4/5/4/5/4/3 · check 0건 · 8회차 오답(지출유형·해지) 반영
SUBJECT1_ROUND9: list[str] = [
    # Part1 (5)
    "1:1:1:exam:20",
    "1:1:2:exam:10",
    "1:1:3:exam:12",
    "1:1:4:exam:12",
    "1:1:1:exam:21",
    # Part2 (4)
    "1:2:1:exam:17",
    "1:2:2:exam:14",
    "1:2:3:exam:11",
    "1:2:1:exam:18",
    # Part3 (5)
    "1:3:1:exam:14",
    "1:3:2:exam:14",
    "1:3:3:exam:13",
    "1:3:4:exam:12",
    "1:3:1:exam:15",
    # Part4 (4)
    "1:4:1:exam:15",
    "1:4:2:exam:13",
    "1:4:3:exam:9",
    "1:4:4:exam:3",
    # Part5 (5) — 지출유형·정책
    "1:5:1:exam:18",
    "1:5:2:exam:13",
    "1:5:3:exam:12",
    "1:5:4:exam:13",
    "1:5:5:exam:11",
    # Part6 (4)
    "1:6:1:exam:10",
    "1:6:2:exam:11",
    "1:6:3:exam:10",
    "1:6:4:exam:10",
    # Part7 (3) — 해제/해지
    "1:7:1:exam:15",
    "1:7:2:exam:14",
    "1:7:1:exam:16",
]

# Part 분포 5/5/5/5 · check 2건 · 8회차 오답(원가·파레토·탈락통지)
SUBJECT2_ROUND9: list[str] = [
    # Part1 (5) — check 1
    "2:1:2:exam:31",
    "2:1:2:exam:32",
    "2:1:2:exam:4",
    "2:1:2:exam:9",
    "2:1:2:check:3",
    # Part2 (5) — 학술용역 원가
    "2:2:3:exam:20",
    "2:2:4:exam:7",
    "2:2:3:exam:21",
    "2:2:4:exam:8",
    "2:2:3:exam:22",
    # Part3 (5) — check 1
    "2:3:1:exam:19",
    "2:3:3:exam:15",
    "2:3:4:check:1",
    "2:3:1:exam:22",
    "2:3:3:exam:16",
    # Part4 (5) — 파레토·탈락통지
    "2:4:1:exam:4",
    "2:4:2:exam:4",
    "2:4:3:exam:11",
    "2:4:4:exam:16",
    "2:4:5:exam:4",
]

# Part 분포 8/7/8/7 · cqa 6건 · **2차 lap** exam+미사용 cqa · 적격·공동도급·JIT
SUBJECT3_ROUND9: list[str] = [
    # Part1 (8) — 6 exam + 2 cqa
    "3:1:1:exam:16",
    "3:1:1:exam:2",
    "3:1:1:exam:22",
    "3:1:1:exam:25",
    "3:1:1:exam:6",
    "3:1:1:exam:7",
    "3:1:4:cqa:8",
    "3:1:4:cqa:9",
    # Part2 (7) — 6 exam + 1 cqa
    "3:2:1:exam:22",
    "3:2:1:exam:25",
    "3:2:1:exam:28",
    "3:2:1:exam:29",
    "3:2:1:exam:31",
    "3:2:1:exam:35",
    "3:2:1:cqa:11",
    # Part3 (8) — 6 exam + 2 cqa
    "3:3:1:exam:15",
    "3:3:1:exam:16",
    "3:3:1:exam:19",
    "3:3:1:exam:2",
    "3:3:1:exam:20",
    "3:3:1:exam:23",
    "3:3:1:cqa:16",
    "3:3:1:cqa:18",
    # Part4 (7) — 6 exam + 1 cqa · 공동도급
    "3:4:1:exam:16",
    "3:4:1:exam:2",
    "3:4:1:exam:20",
    "3:4:1:exam:22",
    "3:4:1:exam:23",
    "3:4:1:exam:25",
    "3:4:1:cqa:10",
]

# Part 5/4/5/4/5/4/3 · exam 29 + check 1 · 9회차 오답(계약심의·하도급전자)
SUBJECT1_ROUND10: list[str] = [
    # Part1 (5)
    "1:1:1:exam:22",
    "1:1:2:exam:14",
    "1:1:3:exam:13",
    "1:1:4:exam:14",
    "1:1:1:exam:23",
    # Part2 (4)
    "1:2:1:exam:19",
    "1:2:2:exam:16",
    "1:2:3:exam:12",
    "1:2:1:exam:20",
    # Part3 (5)
    "1:3:1:exam:17",
    "1:3:2:exam:15",
    "1:3:3:exam:14",
    "1:3:4:exam:13",
    "1:3:1:exam:18",
    # Part4 (4) — check 1
    "1:4:1:exam:16",
    "1:4:2:exam:14",
    "1:4:3:check:1",
    "1:4:4:exam:4",
    # Part5 (5)
    "1:5:1:exam:4",
    "1:5:2:exam:1",
    "1:5:3:exam:3",
    "1:5:4:exam:4",
    "1:5:5:exam:2",
    # Part6 (4)
    "1:6:1:exam:11",
    "1:6:2:exam:12",
    "1:6:3:exam:2",
    "1:6:4:exam:2",
    # Part7 (3)
    "1:7:1:exam:17",
    "1:7:2:exam:16",
    "1:7:1:exam:18",
]

# Part 5/5/5/5 · check 2 · 2차 lap Part1(미사용 check + 통합1~8 exam)
SUBJECT2_ROUND10: list[str] = [
    # Part1 (5) — 2차 lap
    "2:1:2:check:4",
    "2:1:2:check:6",
    "2:1:1:exam:1",
    "2:1:1:exam:10",
    "2:1:1:exam:11",
    # Part2 (5) — RFI·고정가격·시장조사
    "2:2:3:exam:23",
    "2:2:3:exam:25",
    "2:2:3:exam:26",
    "2:2:3:exam:27",
    "2:2:3:exam:28",
    # Part3 (5)
    "2:3:1:exam:28",
    "2:3:1:exam:7",
    "2:3:1:exam:8",
    "2:3:1:exam:9",
    "2:3:3:exam:17",
    # Part4 (5) — 4대보험(경비)
    "2:4:1:exam:5",
    "2:4:1:exam:7",
    "2:4:2:exam:5",
    "2:4:2:exam:6",
    "2:4:2:exam:7",
]

# 2차 lap · Part 8/7/8/7 · cqa 4 (13%) · 9회차 오답(청약·하도급·보증·낙찰결격)
SUBJECT3_ROUND10: list[str] = [
    # Part1 (8) — lap2 exam (Part1 unused cqa 없음)
    "3:1:1:exam:1",
    "3:1:1:exam:10",
    "3:1:1:exam:11",
    "3:1:1:exam:12",
    "3:1:1:exam:13",
    "3:1:1:exam:14",
    "3:1:2:exam:1",
    "3:1:3:exam:4",
    # Part2 (7) — 6 exam + 1 cqa
    "3:2:1:exam:1",
    "3:2:1:exam:10",
    "3:2:1:exam:11",
    "3:2:1:exam:12",
    "3:2:1:exam:13",
    "3:2:1:exam:14",
    "3:2:1:cqa:12",
    # Part3 (8) — 6 exam + 2 cqa
    "3:3:1:exam:1",
    "3:3:1:exam:10",
    "3:3:1:exam:11",
    "3:3:1:exam:12",
    "3:3:1:exam:13",
    "3:3:1:exam:14",
    "3:3:1:cqa:20",
    "3:3:1:cqa:21",
    # Part4 (7) — 6 exam + 1 cqa
    "3:4:1:exam:1",
    "3:4:1:exam:10",
    "3:4:1:exam:11",
    "3:4:1:exam:12",
    "3:4:1:exam:13",
    "3:4:1:exam:14",
    "3:4:1:cqa:11",
]

INTEGRATED_SELECTIONS: dict[int, dict[str, list[str]]] = {
    4: {
        "1": SUBJECT1_ROUND4,
        "2": SUBJECT2_ROUND4,
        "3": SUBJECT3_ROUND4,
    },
    5: {
        "1": SUBJECT1_ROUND5,
        "2": SUBJECT2_ROUND5,
        "3": SUBJECT3_ROUND5,
    },
    6: {
        "1": SUBJECT1_ROUND6,
        "2": SUBJECT2_ROUND6,
        "3": SUBJECT3_ROUND6,
    },
    7: {
        "1": SUBJECT1_ROUND7,
        "2": SUBJECT2_ROUND7,
        "3": SUBJECT3_ROUND7,
    },
    8: {
        "1": SUBJECT1_ROUND8,
        "2": SUBJECT2_ROUND8,
        "3": SUBJECT3_ROUND8,
    },
    9: {
        "1": SUBJECT1_ROUND9,
        "2": SUBJECT2_ROUND9,
        "3": SUBJECT3_ROUND9,
    },
    10: {
        "1": SUBJECT1_ROUND10,
        "2": SUBJECT2_ROUND10,
        "3": SUBJECT3_ROUND10,
    },
}

QUESTION_COUNTS = {"1": 30, "2": 20, "3": 30}
